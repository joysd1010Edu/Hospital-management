...
function showDoctorModal(id) {
  ...
  const doc =
    data && data.doctors
      ? data.doctors.find((d) => Number(d.id) === Number(id))
      : null;
  if (!doc) return;
  ...
  container.querySelector(".doc-availability").textContent = doc.available
    ? "Available"
    : "Not Available";
  container.querySelector(".doc-availability").className =
    "doc-availability " + (doc.available ? "text-green-600" : "text-red-600");
  ...
  const specialties = doc.department
  ...
  prof.innerHTML = `
    ...
    <p class="text-sm text-gray-600">${specialties} • ${
    doc.qualification || ""
  }</p>
  `;
